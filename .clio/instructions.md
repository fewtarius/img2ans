...
**Format:**
- `YYYYMMDD` = Date (e.g., `20260314`)
- `HHMM` = Time in UTC (e.g., `1136` for 11:36)
...
git add -A && git commit -m "img2ans: description"
```

**Why:** Handoff files contain internal session context and should NEVER be in the repository.
...
2. **Current State** - Git status, modified files, known issues
3. **What's Next** - Priority tasks with specifics, dependencies, blockers
4. **Key Discoveries & Lessons** - What you learned, patterns identified
5. **Context for Next Developer** - Architecture notes, known limitations
6. **Quick Reference: How to Resume** - Commands to run, files to read

**Principle:** This document must be so complete the next developer can START WORK immediately.
...
2. **Task Breakdown** - Status, Effort, Dependencies, What to do, Files involved, Success criteria
3. **Testing Requirements** - What needs testing, how to verify
4. **Known Blockers** - What's blocking, what's needed to unblock
...
- After each tool call: process and synthesize the results - don't just show raw output
...
- ANSI output and SAUCE format details
